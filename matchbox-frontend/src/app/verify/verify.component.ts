...
import { ValidationEntry } from './verify-entry';
import { ValidationParameter, ValidationParameterDefinition } from './verify-parameter';
...
export class VerifyComponent implements AfterViewInit {
  ...
  constructor(
    ...
    const validateOperationDefinitionPromise = this.client.read({
      resourceType: 'OperationDefinition',
      id: '-s-validate',
    });

    const implementationGuidesPromise = this.client.search({
  ...
  validateResource(filename: string, content: string, contentType: string, selectBestProfile: boolean): void {
    ...
    try {
      ...
      this.currentResource = new UploadedFile(filename, contentType, content, entry.resourceType);
    ...
    if (searchParams.has('resource')) {
      ...
      if (searchParams.has('profile')) {
        ...
        if (this.supportedProfiles.has(profile)) {
          ...
          this.showErrorToast(
            'Unknown profile',
            `The profile '${profile}' is unknown to this server. Please choose another profile from the list.`
          );
          return;
        }
      }
